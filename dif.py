...
class dif:
    ...
    def __init__(
        ...
        *directory,
        ...
        if duplicate_count + similar_count != 0:
            if self.move_to != None:
                ...
                if len(self.lower_quality) != 0:
                    deleted_files = _help._delete_imgs(
                        set(self.lower_quality), silent_del=self.silent_del
                    )
        ...
        if self.similarity == 0:
            print(
                f"Found {duplicate_count} pair(s) of duplicate image(s) in {time_elapsed} seconds."
            )
        else:
            print(
                f"Found {duplicate_count} pair(s) of duplicate and {similar_count} pair(s) of similar image(s) in {time_elapsed} seconds."
            )
    ...
    def _run(self):
        ...
        for count, dir in enumerate(self.directory):
            if count == 0:
                directory_files, skipped_files = _help._list_all_files(
                    dir, self.recursive, self.limit_extensions
                )
                id_by_location = _compute._id_by_location(
                    directory_files, id_by_location=None
                )
            else:
                if len(self.directory) >= 2:
                    if not os.path.normpath(dir) in directory_files:
                        ...
                        id_by_location = _compute._id_by_location(
                            directory_files, id_by_location=id_by_location
                        )
                    else:
                        print(
                            f"Skipped directory {dir} as it is part of another directory provided."
                        )
                else:
                    break
        imgs_matrices, invalid_files = _compute._imgs_matrices(
            id_by_location, self.px_size, self.show_progress
        )
        (
            result,
            exclude_from_search,
    ...
    def _generate_stats(
        ...
        if self.logs:
            invalid_stats = {"count": len(invalid_files), "logs": invalid_files}
            deleted_stats = {"count": len(deleted_files), "logs": deleted_files}
            skipped_stats = {"count": len(skipped_files), "logs": skipped_files}
        else:
        ...
        stats = {
            ...
            "matches_found": {"duplicates": duplicate_count, "similar": similar_count},
...
class _validate:
    ...
    def _directory_type(directory):
        ...
        if all(isinstance(dir, list) for dir in directory):
            ...
            raise ValueError(
                "Invalid directory parameter: directories must be of type list xor string."
            )
    ...
    def _directory_unique(directory):
        if len(set(directory)) != len(directory):
            raise ValueError(
                "Invalid directory parameters: an attempt to compare a directory with itself."
            )
    ...
    def _fast_search(fast_search, similarity):
        ...
        if not isinstance(fast_search, bool):
            raise Exception(
                'Invalid value for "fast_search" parameter: must be of type bool.'
            )
        if similarity > 0:
            fast_search = False
        return fast_search
    ...
    def _similarity(similarity):
        ...
        if similarity in ["low", "normal", "high"]:
            raise Exception(
                'Since difPy v3.0.8, "similarity" parameter only accepts "duplicates" and "similar" as input options.'
            )
            ...
            try:
                ...
                raise Exception(
                    'Invalid value for "similarity" parameter: must be of type float.'
                )
        else:
    ...
    def _px_size(px_size):
        ...
        if not isinstance(px_size, int):
            raise Exception(
                'Invalid value for "px_size" parameter: must be of type int.'
            )
    ...
    def _show_output(show_output):
        ...
        if not isinstance(show_output, bool):
            raise Exception(
                'Invalid value for "show_output" parameter: must be of type bool.'
            )
        return show_output
    ...
    def _move_to(move_to):
        ...
        if not isinstance(move_to, str):
            if not move_to == None:
                raise Exception(
                    'Invalid value for "move_to" parameter: must be of type str or "None"'
                )
        else:
    ...
    def _delete(delete, silent_del):
        ...
        if not isinstance(delete, bool):
            raise Exception(
                'Invalid value for "delete" parameter: must be of type bool.'
            )
            ...
            for id, file in id_by_location.items():
                ...
                try:
                    ...
                    if e.__class__.__name__ == "UnidentifiedImageError":
                        invalid_files[
                            str(Path(file))
                        ] = f"UnidentifiedImageError: file could not be identified as image."
                    else:
...
class _search:
    ...
    def _matches(
        ...
        for number_A, (id_A, matrix_A) in enumerate(imgs_matrices.items()):
            if show_progress:
                _help._show_progress(
                    progress_count, total_count, task="comparing images"
                )
                            ...
                            if mse <= similarity:
                                ...
                                for key in result.keys():
                                    if id_A in result[key]["matches"]:
                                        result[key]["matches"][id_B] = {
                                            "location": str(Path(id_by_location[id_B])),
                                            "mse": mse,
                                        }
                                        check = True
                                if not check:
                                    if id_A not in result.keys():
                                        result[id_A] = {
                                            "location": str(Path(id_by_location[id_A])),
                                            "matches": {
                                                id_B: {
                                                    "location": str(
                                                        Path(id_by_location[id_B])
                                                    ),
                                                    "mse": mse,
                                                }
                                            },
                                        ...
                                        result[id_A]["matches"][id_B] = {
                                            "location": str(Path(id_by_location[id_B])),
                                            "mse": mse,
                                        }
                                if show_output:
                                    _help._show_img_figs(matrix_A, matrix_B, mse)
                                    _help._show_file_info(
                                        str(Path(id_by_location[id_A])),
                                        str(Path(id_by_location[id_B])),
                                    )
                                if fast_search == True:
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Find duplicate or similar images with difPy - https://github.com/elisemercury/Duplicate-Image-Finder"
    )
    parser.add_argument(
        "-D",
        "--directory",
    ...
    parser.add_argument(
        ...
        "--output_directory",
        type=str,
        help="Output directory for the difPy result files. Default is working dir.",
        required=False,
        default=None,
    )
    parser.add_argument(
        ...
        help="Use difPys Fast Search Algorithm.",
        required=False,
        choices=[True, False],
        default=True,
    )
    parser.add_argument(
        ...
        "--recursive",
        type=lambda x: bool(strtobool(x)),
        help="Scan subfolders for duplicate images.",
        required=False,
        choices=[True, False],
        default=True,
    )
    ...
    parser.add_argument(
        "-s",
        "--similarity",
        type=_help._type_str_int,
        help="Similarity grade.",
        required=False,
        default="duplicates",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        "--show_output",
        type=lambda x: bool(strtobool(x)),
        help="Show the compared images in real-time.",
        required=False,
        choices=[True, False],
        default=False,
    )
    parser.add_argument(
        ...
        "--move_to",
        type=str,
        help="Move the lower quality images to a target folder.",
        required=False,
        default=None,
    )
    parser.add_argument(
        ...
        "--delete",
        type=lambda x: bool(strtobool(x)),
        help="Delete all duplicate images with lower quality.",
        required=False,
        choices=[True, False],
        default=False,
    )
    ...
    parser.add_argument(
        ...
        "--logs",
        type=lambda x: bool(strtobool(x)),
        help="Enable log collection for invalid files.",
        required=False,
        choices=[True, False],
        default=False,
    )
